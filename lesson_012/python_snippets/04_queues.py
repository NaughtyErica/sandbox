...
class Fisher(threading.Thread):
    ...
    def __init__(self, name, worms, catcher, *args, **kwargs):
        ...
        self.name = name
        self.worms = worms
        self.catcher = catcher # Рыбак при рождении получил ссылку на очередь,
        # созданную в лодке
    ...
    def run(self):
        for worm in range(self.worms):
            ...
            if fish is None:
                ...
                self.catcher.put(fish) # Толкаем пойманную рыбу в очередь
...
class Boat(threading.Thread):
    ...
    def add_fisher(self, name):
        ...
        # Мы каждому рыбаку передаем ссылку на очередь catcher=self.catcher,
        # в лодке

    def run(self):
        print('Лодка вышла в море...', flush=True)
        for fisher in self.fishers:
            fisher.start() # Рыбаки разошлись по потокам
        while True:
            try:
                # Этот метод у очереди - атомарный и блокирующий,
                # Поток приостанавливается, пока нет элементов в очереди
                fish = self.catcher.get(timeout=1) # Лодка забирает из очереди пойманную кем-то рыбу
                print(f'Приемщик принял {fish} и положил в садок', flush=True)
                self.fish_tank[fish] += 1 # И присовокупляет к общему улову
                self.count_fish += 1
            except queue.Empty:
        ...
        for fisher in self.fishers:
            fisher.join()
        print(f'Лодка возвращается домой с уловом', flush=True)
        for fish in self.fish_tank.keys():
            print(f'{fish} => {self.fish_tank[fish]}')
        print(f'Итого      {boat.count_fish}')
