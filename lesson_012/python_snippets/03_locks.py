...
class Fisher(threading.Thread):
    ...
    def __init__(self, name, worms, fish_tank, semaphore, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.name = name
        self.worms = worms
        self.catched = 0
        self.fish_tank = fish_tank
        self.fish_tank_semaphore = semaphore
    ...
    def run(self):
        for worm in range(self.worms):
            ...
            if fish is not None:
                ...
                sem = self.fish_tank_semaphore.acquire()
                # print(f'{self.name} установил семафор на садок {sem}')
                self.fish_tank[fish] += 1
                # print(f'{self.name} положил {fish} в садок {self.fish_tank[fish]}')
                sem = self.fish_tank_semaphore.release()
                # print(f'{self.name} сбросил семафор с садка {sem}')
                # with self.fish_tank_lock:
                #     self.fish_tank[fish] += 1
                self.catched += 1
...
def main():
    humans = ['Васек', 'Колян', 'Петрович', 'Хмурый', 'Клава', ]
    semaphore_for_fish_tank = threading.BoundedSemaphore()
    fishers = [Fisher(name=name, worms=10000, fish_tank=global_fish_tank, semaphore=semaphore_for_fish_tank) for name in humans]
